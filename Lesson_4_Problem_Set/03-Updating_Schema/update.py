    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            if line['binomialAuthority_label'] != 'NULL':
                ...
                try:
                    data[line['rdf-schema#label']] = re.match(r'(.*?)(?=\()',data[line['rdf-schema#label']]).group().rstrip(' ')
                except:
                    pass
...
def update_db(data, db):
    ...
    for key in data.keys():
        query = db.arachnid.find_one({'label':key})
        print query
        query['classification'] = {'binomialAuthority':data[key]}
        db.arachnid.save(query)
