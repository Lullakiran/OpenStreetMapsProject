    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            for key,value in fields.iteritems():
                ...
                line[key]=line[key].lstrip(' ')
                line[key]=line[key].rstrip(' ')
                
                if key in ['family_label', 'class_label','phylum_label', 'order_label', 'kingdom_label', 'genus_label']:
                    classification[fields[key]] = None if line[key] =='NULL' else line[key]
                    
                else:
                    
                    updated_dict[value] = None if line[key] =='NULL' else line[key]
            try:    
                updated_dict['synonym']= parse_array(updated_dict['synonym'].replace('*',''))
            except:
                pass
            try:
                updated_dict['label']=re.match(r'(.*?)(?=\()',updated_dict['label']).group().rstrip(' ')
            except:
                pass
            #print updated_dict['label']
            if updated_dict['name'] == None:
                updated_dict['name'] = updated_dict['label']
            updated_dict['classification'] = classification
