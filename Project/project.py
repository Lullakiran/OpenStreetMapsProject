...
def correct_postcode(postcode):  # postcodes must start with 06, and must be 5 digits long
    if len(postcode) != 5:  # found short postcodes like 0600, 65 in the data
        return None
    elif re.match(r'(06\d\d\d)', postcode) is None:  # check if all digits (i.e. not Esma 3 Sokak) and starts with 06
...
def capitalize_turkish_title(address):
    ...
    connectors = ['ve', 'de', 'ile', 'ki', 'ya', 'ya da']
        # this words should not be capitalized
    for word in address.split():

        capitalized_address += word + \
            ' ' if word in connectors else word.title() + ' '
    # if address.islower():
    # print 'before capitalization: ', address, '\t after capitalization: ',
    # capitalized_address
...
def shape_element(element):
    ...
    if element.tag == "node" or element.tag == "way":
        ...
        try:
            node['pos'] = [
                float(get_value(element, 'lat')), float(get_value(element, 'lon'))]
        except:
            pass
        ...
        for i in element:
            ...
            if element.tag == "way":
                try:
                    node.setdefault('node_refs', []).append(
                        i.attrib['ref'])  # http://stackoverflow.com/questions/12905999/python-dict-how-to-create-key-or-append-an-element-to-key
                except:
                    pass

            try:
                if "addr" in i.attrib['k']:
                    addr, field = lower_colon.match(
                        i.attrib['k']).group().split(':')
                    length = len(
                        lower_colon.match(i.attrib['k']).group().split(':'))
                    v = i.attrib['v']

                    if length <= 2 and addr == 'addr' and problemchars.match(v) is None:
                        if field == 'postcode':
...
clean_ankara = process_map(input_ankara)
