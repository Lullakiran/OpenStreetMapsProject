...
def make_pipeline():
    ...
    pipeline = [  { "$match" : { "country":"India"}},
                  { "$unwind" : "$isPartOf"},
                  {"$group" : {"_id":"$isPartOf",   "population":{ "$avg": "$population"}}}   ,
                    { "$sort" : {"population" :-1}},  # there are some problems here. I think this question is problematic
                                                      #some districts are either similar (i.e 'Mumbai Suburban district' and 'Mumbai City District'
                                                      #, or included  in one in another (i.e. a district is a part of a region)
               { "$group" : {"_id":"India Regional City Population Average", "avg": {"$avg":"$population"}}}
                ]
    return pipeline
